...
if delete_spectra_q == 'y' : 
    ...
    while delete_mult_spectra == 'y':
        delete_spectra_a = int(input('Which spectrum should be removed from 1 - ' + str(intensities.columns.size) + ': '))
        if delete_spectra_a >=1 and delete_spectra_a <= intensities.columns.size :
            intensities.drop(columns = [int(delete_spectra_a)], axis=1, inplace= True) 
...
while maxima_ok == 'n':
    ...
    n = int(input('Noise level (integer value) of mean spectrum (<10 for graphitized carbons, > 10 for amorphous carbons): ')) * 20 
    #n = 140
    #slice in x in which search for maxima is done
    maxima = mean_intensity_table.iloc[500:3400]
    ...
    G_D_Peak = maxima_numbers.loc[maxima_numbers['Mean_Intensity'] > 0.5] 
    ...
    plt.scatter(maxima_numbers['Wavenumber'], maxima_numbers['Mean_Intensity'], label = 'Local Maxima', c='r')
    #axis labels
    ...
    #plot showing
    plt.show()
    maxima_ok = input('Are the maxima found correct?(y/n): ')
...
maxima_numbers.set_index('Wavenumber', inplace = True)
#show only significant digits
maxima_numbers = maxima_numbers.round(3)
...
with open('maxima_data.txt', 'w') as maxima_data_file:
    ...
    maxima_numbers_string = maxima_numbers.to_string()
    maxima_data_file.write(maxima_numbers_string)
